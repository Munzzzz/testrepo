...
def plot_scatterplots(df, target, ncols=2):
    ...
    _hide_unused_axes(fig, axs, n, nrows, ncols)
    plt.tight_layout()
    plt.savefig("eda_scatterplots.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_correlation_heatmaps(df, methods=('pearson', 'spearman', 'kendall')):
    for method in methods:
        ...
        mask = np.triu(np.ones_like(correlations, dtype=bool), k=1)   # upper triangle is redundant
        sns.heatmap(correlations, mask=mask, annot=True, cmap='coolwarm',
                   vmin=-1, vmax=1, center=0, square=True, linewidths=1,
                   cbar_kws={"shrink": 0.8}, annot_kws={"size": 9}, fmt='.2f',
                   cbar=True, ax=ax)
...
# One figure per model: 2x2 grid of R2 / RMSE / MAPE / SI vs fold count
for spec in models:
...
for spec in models:
    ...
    plt.tight_layout()
    plt.savefig(f"learning_curve_{spec.name.replace(' ', '_')}.png",
...
plt.tight_layout()
plt.savefig("test_r2_comparison.png", dpi=300, bbox_inches='tight',
           facecolor='white', edgecolor='none')
plt.show()
plt.close()
...
def plot_actual_vs_predicted(models, X, y, dataset_label):
    ...
    for spec in models:
        ...
        plt.tight_layout()
        plt.savefig(f"{spec.name.replace(' ', '_')}_{dataset_label}.png",
...
def plot_bias_variance_curve(spec, param_name, values, xlabel, n_bootstrap=10,
    ...
    ax.grid(True, linestyle='--', alpha=0.4, color='gray')
    plt.tight_layout()
    plt.savefig(f"bias_variance_curve_{spec.name.replace(' ', '_')}.png", dpi=300,
...
for spec, explanation in zip(models, explanations):
    ...
    plt.tight_layout()
    plt.savefig(f"shap_bar_{spec.name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
for spec, explanation in zip(models, explanations):
    ...
    plt.tight_layout()
    plt.savefig(f"shap_beeswarm_{spec.name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
for spec, explanation in zip(models, explanations):
    ...
    plt.tight_layout()
    plt.savefig(f"shap_violin_{spec.name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
for spec, explanation in zip(models, explanations):
    ...
    plt.tight_layout()
    #plt.savefig(f"shap_waterfall_{spec.name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
for spec, explanation in zip(models, explanations):
    ...
    plt.tight_layout()
    #plt.savefig(f"shap_dependence_{spec.name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
X_pdp      = x_train.iloc[_pdp_idx] if hasattr(x_train, "iloc") else pd.DataFrame(X_tr[_pdp_idx], columns=feature_names)
...
def plot_ice(specs, X_plot, features=None, n_cols=N_COLS):
    ...
    for spec in specs:
        ...
        fig.tight_layout()
        fig.savefig(f"ice_{spec.name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
        plt.show()
        plt.close(fig)
...
def plot_pdp_2way(specs, X_plot, pairs=None, n_cols=N_COLS):
    ...
    for spec in specs:
        ...
        for idx, (ax, pd_result) in enumerate(zip(axes_flat[:len(pairs)], display.pd_results)):
            ax.clear()
        ...
        fig.savefig(f"pdp_2way_{spec.name.replace(' ', '_')}.png",
                   dpi=300, bbox_inches='tight')
        plt.show()
        plt.close(fig)
